...
function mostrarProducto(newcard) {
  ...
  card.innerHTML = `
        ...
        <h2>${newcard.title}</h2>
        <p id="titulo">Precio: $${
          newcard.price ? newcard.price.toFixed(2) : "N/A"
        }</p>   
...
function calcularTotalCarrito(carrito) {
  ...
  totalCarritoElement.textContent = `Total del carrito: $${total.toFixed(
    2
  )}     `;
...
document.addEventListener("click", function (event) {
  if (event.target && event.target.classList.contains("btn")) {
    ...
    const price = parseFloat(
      productagr.querySelector("p").textContent.replace("Precio: $", "")
    );
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  carrito.forEach(function (producto, index) {
    ...
    function calcularTotalCarrito() {
      ...
      const totalCarritoElement = document.getElementById("total-carrito");
      totalCarritoElement.textContent = `Total del carrito: $${totalCarrito.toFixed(
        2
      )}   `;
    }
